...
// ----------------------------------------------------------------------------------------------------
// TUGAS 2: PERSONNEL SEVERITY MATRIX COMPONENT
// ----------------------------------------------------------------------------------------------------
interface PersonnelSeverityMatrixProps {
  data: BugRecord[];
  filterType: string;
  selectedSeverity?: string;
  onChartClick?: (type: string, value: string) => void;
...
export function PersonnelSeverityMatrix({ data = [], filterType = "All", selectedSeverity = "All", onChartClick, onExportPDF, onExportMatrix, isExporting }: PersonnelSeverityMatrixProps) {
  const [trendYear, setTrendYear] = React.useState<string | null>(null);
  const [varianceYear, setVarianceYear] = React.useState<string | null>(null);

  // Derive intensity & severity distribution indices Dynamically based on incoming data
  const severityCounts = React.useMemo(() => {
    return data.reduce((acc: any, bug) => {
      const sev = bug.severity || "Minor";
      acc[sev] = (acc[sev] || 0) + 1;
      return acc;
    }, {});
  }, [data]);
  ...
  // Derive personnel rankings scoring index dynamically based on incoming data
  const devStats = React.useMemo(() => {
    ...
    data.forEach((bug) => {
      ...
      stats[dev].totalScore += (Number(bug.bugScore) || 0);

      const sev = bug.severity || "Minor";
      if (sev === "Critical") stats[dev].criticalCount += 1;
      else if (sev === "Major") stats[dev].majorCount += 1;
      else if (sev === "Minor") stats[dev].minorCount += 1;
      else if (sev === "Trivia") stats[dev].triviaCount += 1;
      else if (sev === "Recurring") stats[dev].recurringCount += 1;
    });

    return Object.values(stats).sort((a: any, b: any) => b.totalScore - a.totalScore);
  }, [data]);

  // Derive monthly / yearly trend volume dynamically from incoming data
  const getTrendData = React.useCallback((targetYear: string | null, dataToUse: BugRecord[]) => {
    if (!targetYear) {
      // Level 1: Yearly
      const yearlyMap = dataToUse.reduce((acc: any, bug) => {
        const periodParts = bug.periode?.split('-');
      ...
      const monthlyMap = dataToUse
        ...
        .reduce((acc: any, bug) => {
          ...
          const sev = bug.severity || "Minor";
          acc[month][sev] = (acc[month][sev] || 0) + 1;
          acc[month].value++;
          return acc;
        }, {});
  ...
  const trendChartData = React.useMemo(() => getTrendData(trendYear, data), [trendYear, data, getTrendData]);
  const varianceChartData = React.useMemo(() => getTrendData(varianceYear, data), [varianceYear, data, getTrendData]);

  const isBugView = filterType === 'Bug';
  const isCRView = filterType === 'CR';
  
  const activeColor = isBugView ? "#F59E0B" : isCRView ? "#10B981" : "#3B82F6";
  const activeGradient = isBugView ? "url(#color-Bug)" : isCRView ? "url(#color-CR)" : "url(#color-All)";
  const activeTitle = filterType === 'All' ? "SIT LEDGER" : filterType === 'Bug' ? "BUG" : "CHANGE REQUEST";
  const activeSeriesLabel = filterType === 'All' ? "Items" : filterType === 'Bug' ? "Bug" : "Change Request";
      ...
      <div id="chart-matrix-group" className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-0.5">
                {filterType === "All" ? "SIT Ledger (Bug + CR)" : `${filterType} Perspective`} Analysis
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div id="chart-dev-ranking" className="glass-card p-8 relative overflow-hidden group">
            ...
            <div className="h-[280px] w-full min-h-0 min-w-0">
              {devStats.length === 0 ? (
                <div className="h-full flex items-center justify-center text-slate-500 text-xs">No personnel data mapped for selection.</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart 
                    data={devStats.slice(0, 10)} 
          ...
          <div id="chart-severity-index" className="glass-card p-8 relative overflow-hidden group">
            ...
            <div className="h-[280px] w-full flex items-center justify-center min-h-0 min-w-0">
              {severityData.length === 0 ? (
                <div className="h-full flex items-center justify-center text-slate-500 text-xs">No severity distribution mapped.</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                ...
                <linearGradient id="color-All" x1="0" y1="0" x2="0" y2="1">
